/** Логика навигации по шагам формы калитки типа 9. Автономна от type2Navigation.
 *  Type9 поддерживает только профнастил и только одну сторону (без двора).
 */

import { WICKET_MENU_ITEMS } from './wicketMenuConfig'
import type { WicketSharedNavState } from './wicketNavTypes'

export type Type9NavState = WicketSharedNavState

const MENU_ITEMS = WICKET_MENU_ITEMS

export function getNextPage(from: string, state: Type9NavState): string | null {
  switch (from) {
    case 'page2':
...
export function getPrevPage(from: string, state: Type9NavState): string | null {
  switch (from) {
    case 'page2_facade_profnastil':
      return 'page2'
    case 'page5':
