...
const materialOptions = [
  { label: 'Штакетник', value: 'Штакетник' },
]
...
onMounted(async () => {
  ...
  // Дефолт материала «Штакетник» (единственная опция) — выставляем, если ещё не выбран
  if (!calc.materialFacadeGlob.value) {
    calc.materialFacadeGlob.value = 'Штакетник'
  }
  if (calc.fillSide.value === 'Две стороны' && !calc.materialYardGlob.value) {
    calc.materialYardGlob.value = 'Штакетник'
  }
  await save()
})
</script>
