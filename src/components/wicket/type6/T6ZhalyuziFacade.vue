...
import { getZhalyuziTable, saveWicketData, recalculate, type ZhalyuziRow, type SidingPagination } from '../../../app/api/wicket'
import T6FilterDropdown from './T6FilterDropdown.vue'
import { wicketZhalyuziFilterDataUrl } from '../shared/wicketMaterialApiPaths'
...
const zhalyuziFilterDataUrl = computed(() => wicketZhalyuziFilterDataUrl(calc.modelId.value))
...
const rows         = ref<ZhalyuziRow[]>([])
const selectedRow  = ref<ZhalyuziRow | null>(null)
...
async function loadTable(page = currentPage.value) {
  ...
  try {
    const result = await getZhalyuziTable(calc.modelId.value, filters, page)
    rows.value        = result.table
    pagination.value  = result.pagination
    currentPage.value = result.pagination.page
  } catch (e) {
    console.warn('getZhalyuziTable недоступен:', e)
    rows.value = []
  } finally {
...
function restoreFromCalc(): ZhalyuziRow | null {
...
async function save(row: ZhalyuziRow) {
...
async function selectRow(row: ZhalyuziRow) {
...
function isSelected(row: ZhalyuziRow) {
...
onMounted(async () => {
  calc.setActivePage('page2_facade_zhalyuzi')
  await loadTable()
})
</script>
