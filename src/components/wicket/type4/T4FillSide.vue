...
onMounted(async () => {
  ...
  // Дефолт материала «Ламель» (единственная опция) — выставляем, если ещё не выбран
  if (!calc.materialFacadeGlob.value) {
    calc.materialFacadeGlob.value = 'Ламель'
  }
  if (calc.fillSide.value === 'Две стороны' && !calc.materialYardGlob.value) {
    calc.materialYardGlob.value = 'Ламель'
  }
  await save()
})
</script>
